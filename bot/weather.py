...
async def get_weather(location: str) -> str:
    async with request('GET', domain + weather_request + quote(location), headers={'User-Agent': user_agent}) as resp:
    ...
    if title != 'Яндекс.Погода':
        ...
        exact_location = node.text()
        async with request('GET', href, headers={'User-Agent': user_agent}) as resp:
    ...
    wind_info = card.css_first('div.fact__props').css_first('div.fact__wind-speed').css_first('div.term__value')
    now_wind = wind_info.css_first('span.wind-speed').text() + ' ' + wind_info.css_first('span.fact__unit').text()

    day_info = HTMLParser(weather_text).css_first('div.forecast-briefly').css_first('ul.swiper-wrapper')
    # print(day_info.html)
    slide = None
...
if __name__ == '__main__':
    print(asyncio.get_event_loop().run_until_complete(get_weather('москва')))
